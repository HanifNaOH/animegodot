...
func _execute_async(args: Array = []) -> void:
	...
	if result is Signal:
		_completion_signal = result
		_completion_callback = func(_unused = null): _on_async_finished(execution_id)
		result.connect(_completion_callback, CONNECT_ONE_SHOT)
	else:
...
func _clear_completion_connection() -> void:
	if _completion_signal is Signal and _completion_callback.is_valid() and _completion_signal.is_connected(_completion_callback):
		_completion_signal.disconnect(_completion_callback)
	_completion_signal = null
	_completion_callback = Callable()
