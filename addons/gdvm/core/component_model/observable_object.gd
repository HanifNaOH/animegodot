...
## Notify listeners and apply the setter write through a callback.
## This preserves emit-before-write for properties backed by a separate field.
## The callback is invoked only when old_value and new_value differ. It must
## write backing storage rather than assigning the public property again.
func set_property_and_apply(property_name: StringName, old_value, new_value, apply: Callable) -> bool:
	assert(apply.is_valid(), "ObservableObject: apply callback must be valid.")
	if set_property(property_name, old_value, new_value):
		apply.call()
		return true
	return false
