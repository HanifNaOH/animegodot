...
accompanying commands and messaging are supporting toolkit features.
...
For properties with separate backing storage, the callback helper preserves
the same notification order:

```gdscript
var _greeting: String = "hello"
var greeting: String:
	get:
		return _greeting
	set(v):
		set_property_and_apply(&"greeting", _greeting, v, func(): _greeting = v)
```
...
The current GDVM release is defined in [`VERSION`](VERSION). The runtime is
...
For nested ViewModel properties, use `bind_path()`:

```gdscript
binder.bind_path(%HealthLabel, "player.stats.health", "text", {
	"converter": &"str",
})
```

Nested bindings subscribe to each observable object in the path and reconnect
when an intermediate object is replaced. Existing `bind()` calls remain for
direct properties.
...
When `on_removed` is provided, ownership of removed rows is transferred to
that callback. The callback must call `free()` or `queue_free()` when the row
is no longer needed. Without `on_removed`, the binder frees removed rows.
