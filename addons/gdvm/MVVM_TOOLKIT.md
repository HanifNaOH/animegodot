...
The current release is defined in [`VERSION`](VERSION). GDVM follows SemVer:
major releases may break the public API, minor
...
For nested ViewModel properties, `GdvmBinder.bind_path()` can follow an
observable chain and reconnect when an intermediate ViewModel is replaced:

```gdscript
binder.bind_path(health_label, "player.stats.health", "text")
```


Use `set_property_and_apply()` when a property has separate backing storage:
...
var health: int:
	get:
		return _health
	set(v):
		set_property_and_apply(&"health", _health, v, func(): _health = v)
```
...
```gdscript
# Asking for stats:
var msg = Gdvm.RequestMessage.RequestMessage.new()
...
## Examples
See `examples/_11_task_manager/` and `examples/_12_simulation_dashboard/` for
complete code-first MVVM examples.